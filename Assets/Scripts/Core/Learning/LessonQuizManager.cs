...
public class LessonQuizManager : MonoBehaviour
{
    ...
    [Header("Simple GPA (per correct answer)")]
    [SerializeField] private bool useSimpleGpaPerCorrectAnswer = true;
    [SerializeField, Min(0f)] private float gpaPerCorrectAtHome = 0.1f;
    [SerializeField, Min(0f)] private float gpaPerCorrectInClass = 0.2f;
    [SerializeField, Min(0f)] private float gpaMax = 4.0f;
    ...
    private void ApplySimpleGpaPerCorrect(LessonQuizData data, int wrongCount, LessonQuizStudyContext ctx)
    {
        if (StatManager.Instance == null) return;
        if (data == null || data.questions == null) return;

        float perCorrect = 0f;
        if (ctx == LessonQuizStudyContext.EveningHome) perCorrect = Mathf.Max(0f, gpaPerCorrectAtHome);
        else if (ctx == LessonQuizStudyContext.MorningClassDesk) perCorrect = Mathf.Max(0f, gpaPerCorrectInClass);

        if (perCorrect <= 0f) return;

        int total = Mathf.Max(0, data.questions.Count);
        wrongCount = Mathf.Clamp(wrongCount, 0, total);
        int correct = Mathf.Clamp(total - wrongCount, 0, total);

        float delta = correct * perCorrect;
        if (delta <= 0f) return;

        float max = Mathf.Max(0f, gpaMax);
        StatManager.Instance.gpa = Mathf.Clamp(StatManager.Instance.gpa + delta, 0f, max);
    }
    ...
    private void ApplyRewardsAndStats(LessonQuizData data, float score01, int wrongCount, LessonQuizStudyContext ctx)
    {
        ...
        // NEW: GPA theo số câu đúng (home/class), clamp max 4.0
        if (useSimpleGpaPerCorrectAnswer &&
            (ctx == LessonQuizStudyContext.EveningHome || ctx == LessonQuizStudyContext.MorningClassDesk))
        {
            ApplySimpleGpaPerCorrect(data, wrongCount, ctx);
        }
        ...
        if (gpaMax > 0f)
        {
            StatManager.Instance.gpa = Mathf.Clamp(StatManager.Instance.gpa, 0f, gpaMax);
        }
