...
public static class ChoiceEventRuntime
{
    ...
    private static void ApplyChoice(ChoiceEventData data, ChoiceOption option)
    {
        ...
        if (sm != null)
        {
            sm.gpa = Mathf.Max(0f, sm.gpa + option.gpaDelta);
        ...
        {
            string choiceKey = !string.IsNullOrWhiteSpace(option.choiceFlagId)
                ? option.choiceFlagId
                : (!string.IsNullOrWhiteSpace(data.eventId) ? $"{data.eventId}:{option.label}" : null);

            if (option.tone == ChoiceTone.Bad)
            {
                SemesterProgressManager.Instance.RegisterBadChoice(choiceKey, Mathf.Max(1, option.tonePoints));
            }
            else if (option.tone == ChoiceTone.Good)
            {
                SemesterProgressManager.Instance.RegisterGoodChoice(choiceKey, Mathf.Max(1, option.tonePoints));
            }
        }
        ...
        if (SemesterProgressManager.Instance != null)
        {
            SemesterProgressManager.Instance.CheckAndTriggerBadHabit();
