...
public partial class StoryEventManager : MonoBehaviour
{
    ...
    private float dropoutEndingCheckCooldown;

    private void TryTriggerDropoutEnding()
    {
        ...
        SemesterProgressManager.Instance.TryTriggerDropout();
    }
    ...
    /// <summary>
    /// Ngày không có tiết trên lịch điện thoại (vd. cuối tuần) hoặc event bật nghỉ điểm danh — không popup nhắc giờ vào lớp.
    /// </summary>
    private bool ShouldSuppressClassAttendanceRemindersToday()
    {
        if (ActiveEvent != null && ActiveEvent.skipClassAttendanceToday)
        {
            return true;
        ...
        if (PhoneSystem.Instance == null || GameTimeManager.Instance == null)
        {
            return false;
        }
        ...
        int sem = GameTimeManager.Instance.Semester;
        int day = GameTimeManager.Instance.DayInSemester;
        ClassScheduleEntry sched = PhoneSystem.Instance.GetScheduleForDay(sem, day);
    ...
    private void TryShowClassAttendanceReminder()
    {
        if (!enableDailyClassAttendance) return;
        if (GameTimeManager.Instance == null) return;
        if (ShouldSuppressClassAttendanceRemindersToday()) return;
