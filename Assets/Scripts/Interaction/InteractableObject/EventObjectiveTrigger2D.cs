...
public class EventObjectiveTrigger2D : MonoBehaviour
{
    ...
    [SerializeField] private bool blockClassEntryOutsideClassHours = true;
    ...
    private bool lastClassEntryOpen = true;

    private void Awake()
    {
        cachedCollider = GetComponent<Collider2D>();
        RefreshClassEntryBlocker();
    }
    ...
    {
        if (!blockClassEntryOutsideClassHours) return;
        if (!registerAttendClassOnEnter) return;
        if (lastClassEntryOpen) return;
        if (collision == null || collision.collider == null) return;
        if (!collision.collider.CompareTag("Player")) return;

        TryShowClassBlockedDialogue();
    }

    private void RefreshClassEntryBlocker()
    {
        if (!blockClassEntryOutsideClassHours) return;
        if (!registerAttendClassOnEnter) return;
        ...
        {
            open = StoryEventManager.Instance.CanEnterClassAreaNow(out _, out _);
        }

        if (open == lastClassEntryOpen && cachedCollider.isTrigger == open) return;

        lastClassEntryOpen = open;
        cachedCollider.isTrigger = open;
    }
    ...
    private void TryShowClassBlockedDialogue()
    {
        if (!showDialogueWhenClassBlocked) return;
        if (StoryEventManager.Instance == null) return;

        bool open = StoryEventManager.Instance.CanEnterClassAreaNow(out string statusText, out var result);
        if (open) return;
